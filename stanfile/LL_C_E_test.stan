...
parameters {
  //array[P] vector[A] ap;                    // alpha_x
  array[P] simplex[A] bp; 
  array[P] real cp;
  vector<lower=0>[n_eq] cp2s;
...
transformed parameters  {
  ...
  for(p in 1:P){
    ...
    kp_e[p][2:T] = kps[p];
    kp[p] = kp_e[p] + eq[p]' * cp2[eq_index[p]];
  }
  
  for(p in 1:P){
    for (j in 1:T){
      for (i in 1:A) {
      logm[AT*(p-1) + A * (j-1) + i] = loge[AT*(p-1) + A * (j-1) + i] + bp[p][i] * kp[p][j] + theta[i][p];
      }
    }
  }
...
generated quantities {
  ...
  for(p in 1:P){
    ...
    for (t in 1:Tf){
      for (i in 1:A) {
      mfore[pos1] = exp(bp[p][i] * kp_p[p][t] + theta[i][p]);
      pos1 += 1;
      }
    } 
