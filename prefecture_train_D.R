...
death2 <- expos2 <- rate2 <- real_rate2 <- array(0,c(nrow(Hokkaido$death$total$raw[,34:63]), ncol(Hokkaido$death$total$raw[,34:63]), 46))
test_rate2 <- test_death2 <- test_expos2 <-array(0, c(nrow(Hokkaido$death$total$raw[,64:76]), ncol(Hokkaido$death$total$raw[,64:76]), 46))
mu_a <- matrix(0, ncol = 46, nrow = nrow(Hokkaido$death$total$raw[,34:63]))
mu_b <- matrix(0, ncol = 46, nrow = ncol(Hokkaido$death$total$raw[,34:63]))
for (s in 1:46) {
  death2[,,s] <- get(state[s])$death$total$raw[,34:63]
  expos2[,,s] <- get(state[s])$expos$total$raw[,34:63]
  # rate2[,,s] = r = get(state[s])$rate$total$raw[2:51,]
  real_rate2[,,s] = r = death2[,,s]/expos2[,,s]
  test_death2[,,s] = get(state[s])$death$total$raw[,64:76]
  test_expos2[,,s] = get(state[s])$expos$total$raw[,64:76]
  test_rate2[,,s] = get(state[s])$death$total$raw[,64:76]/get(state[s])$expos$total$raw[,64:76]
  mu_a[,s] <- log(rowMeans(r))
  mu_b[,s] <- log(colMeans(r))
  # mu_a[,s] <- log(rowSums(death2[,,s])/rowSums(expos2[,,s]))
  # mu_b[,s] <- log(colSums(death2[,,s])/colSums(expos2[,,s]))
...
rdsfile <- "output/prefecture/D/prefecture_LC_O.RDS"
...
rdsfile <- "output/prefecture/D/prefecture_LC_C.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata2,
                      chains = 4,
...
rdsfile <- "output/prefecture/D/prefecture_LC_O_2.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata2,
                      chains = 4,
...
rdsfile <- "output/prefecture/D/prefecture_LC_C_2.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata2,
                      chains = 4,
...
rdsfile <- "output/prefecture/D/prefecture_LL_O.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata2,
                      chains = 4,
...
rdsfile <- "output/prefecture/D/prefecture_LL_O_2.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata2,
                      chains = 4,
...
rdsfile <- "output/prefecture/D/prefecture_LL_C.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata2,
                      chains = 4,
...
rdsfile <- "output/prefecture/D/prefecture_LL_C_2.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata2,
                      chains = 4,
...
rdsfile <- "output/prefecture/A/prefecture_LC_O_E_2.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata,
                      chains = 4,
...
rdsfile <- "output/prefecture/A/prefecture_LC_C_E_2.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata,
                      chains = 4,
...
rdsfile <- "output/prefecture/A/prefecture_LL_O_E_2.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata,
                      chains = 4,
...
rdsfile <- "output/prefecture/A/prefecture_LL_C_E_2.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata,
                      chains = 4,
...
getsummarymedian_all <- function(folder = "output/prefecture/"){
  ...
  for (name in files) {
    ...
    Tfore = 50
    for (i in 1:5) {
      m_i[[i]] <- array(m[,i], c(35, 30, 46))
      mfore_i[[i]] <- array(mfore[,i], c(35, Tfore, 46))
      mf[[i]] <- array(0,c(35, 30 + Tfore, 46))
      for (p in 1:46) {
    ...
    RMSE_1 = rmse(as.vector(test_rate2[,2:10,]), as.vector(mfore_i[[3]][,2:10,]))
    MAE_1 = mae(as.vector(test_rate2[,2:10,]), as.vector(mfore_i[[3]][,2:10,]))
    R2_1 = r_squared(as.vector(test_rate2[,2:10,]), as.vector(mfore_i[[3]][,2:10,]))
    
    RMSE_2 = rmse(as.vector(test_rate2), as.vector(mfore_i[[3]][,1:13,]))
    MAE_2 = mae(as.vector(test_rate2), as.vector(mfore_i[[3]][,1:13,]))
    R2_2 = r_squared(as.vector(test_rate2), as.vector(mfore_i[[3]][,1:13,]))
    
    # 汇总统计量
    comp <- matrix(c(WAIC, LOOIC, RMSE_1, MAE_1, R2_1, RMSE_2, MAE_2, R2_2), nrow = 1)
    colnames(comp) <- c("WAIC", "LOOIC","RMSE_11_19", "MAE_11_19", "R2_11_19", "RMSE_10_22", "MAE_10_22", "R2_10_22")
...
allresult <- getsummarymedian_all("output/prefecture/D/")
save(allresult, file = "compare/prefecture_D.RData")
