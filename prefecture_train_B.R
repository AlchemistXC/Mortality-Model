...
death2 <- expos2 <- array(0,c(nrow(Hokkaido$death$total$raw), 33, 46))
rate2 <- array(0, c(nrow(Hokkaido$death$total$raw), 33, 46))
real_rate2 <- array(0, c(nrow(Hokkaido$death$total$raw), 33, 46))
test_rate2 <- array(0, c(nrow(Hokkaido$death$total$raw), 10, 46))
mu_a <- matrix(0, ncol = 46, nrow = nrow(Hokkaido$death$total$raw))
mu_b <- matrix(0, ncol = 46, nrow = 33)
for (s in 1:46) {
  death2[,,s] <- get(state[s])$death$total$raw[,44:76]
  expos2[,,s] <- get(state[s])$expos$total$raw[,44:76]
  # rate2[,,s] = r = get(state[s])$rate$total$raw[2:51,]
  real_rate2[,,s] = r = death2[,,s]/expos2[,,s]
...
rdsfile <- "output/prefecture/B/prefecture_LC_O_E_2.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata,
                      chains = 4,
...
rdsfile <- "output/prefecture/B/prefecture_LC_C_E_2.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata,
                      chains = 4,
...
rdsfile <- "output/prefecture/B/prefecture_LL_O_E_2.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata,
                      chains = 4,
...
rdsfile <- "output/prefecture/B/prefecture_LL_C_E_2.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata,
                      chains = 4,
...
getsummarymedian_all <- function(folder = "output/prefecture/"){
  ...
  for (name in files) {
    ...
    Tfore = 10
    for (i in 1:5) {
      m_i[[i]] <- array(m[,i], c(35, 33, 46))
      mfore_i[[i]] <- array(mfore[,i], c(35, Tfore, 46))
      mf[[i]] <- array(0,c(35, 33 + Tfore, 46))
      for (p in 1:46) {
...
allresult <- getsummarymedian_all("output/prefecture/B/")
save(allresult, file = "compare/prefecture_B.RData")
