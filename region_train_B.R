...
death <- expos <- rate <- array(0,c(nrow(region_Hokkaido$death$total$raw), 33, 8))
test_rate <- array(0,c(nrow(region_Hokkaido$death$total$test), ncol(region_Hokkaido$death$total$test), 8))
mu_a <- matrix(0, ncol = 8, nrow = nrow(region_Hokkaido$death$total$train))
mu_b <- matrix(0, ncol = 8, nrow = 33)

for (r in 1:8) {
  death[,,r] <- get(region[r])$death$total$raw[,44:76]
  expos[,,r] <- get(region[r])$expos$total$raw[,44:76]
  rate[,,r] = get(region[r])$rate$total$raw[,44:76]
...
rdsfile <- "output/region/B/region_LC_O_E_2.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata,
                      chains = 4,
...
rdsfile <- "output/region/B/region_LC_C_E_2.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata,
                      chains = 4,
...
rdsfile <- "output/region/B/region_LL_O_E_2.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata,
                      chains = 4,
...
rdsfile <- "output/region/B/region_LL_C_E_2.RDS"
if(!file.exists(rdsfile)){
  model = cmdstan_model(stanfile, cpp_options = list(stan_threads = TRUE))
  fit <- model$sample(standata,
                      chains = 4,
...
getsummarymedian_all <- function(folder = "output/region/"){
  ...
  for (name in files) {
    ...
    Tfore = 10
    for (i in 1:5) {
      m_i[[i]] <- array(m[,i], c(35, 33, 8))
      mfore_i[[i]] <- array(mfore[,i], c(35, Tfore, 8))
      mf[[i]] <- array(0,c(35, 33 + Tfore, 8))
      for (p in 1:8) {
...
allresult <- getsummarymedian_all("output/region/B/")
save(allresult, file = "compare/region_B.RData")
